...
the CIFAR10 dataset, and can be adapted to different input and output shapes. 
"""
...
class TransportT(torch.nn.Module):
    def __init__(self, input_shape = [3,32,32], zdim = 100):
        ...
        self.fc_T = nn.Sequential(
            nn.Conv2d(conv_channel*8, zdim, 2, 1, 0),
        )
...
class TransportG(torch.nn.Module):
    def __init__(self, zdim = 100, output_shape = [1,32,32]):
        ...
        self.S = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(256, 256, 11, stride=1, padding=5),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(256, output_shape[0], 5, stride=1, padding=2),
